...
import {addFavoriteArticle, removeFavoriteArticle} from "../../../api/articles";
...
const ArticleHeader = ({ favorited = false, status, title, tagList, favoritesCount, slug}) => {
    ...
    const addLike = async () => {
        try {
            ...
            console.log(await addFavoriteArticle(slug))
    ...
    const removeLike = async () => {
        try {
            dispatch(statusActions.search())
            console.log(await removeFavoriteArticle(slug))
            dispatch(statusActions.ok())
            setLiked(false)
            setLikeCount(state => state - 1 )
        } catch {
