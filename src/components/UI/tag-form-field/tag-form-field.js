import classes from "./tag-form-field.module.css"
...
const TagForm = ({ defaultValue }) => {
    ...
    const validateTags = (_, value) => {
        if (value.length > 16) {
            return Promise.reject(new Error('Tag length must not exceed 16 characters'));
        }
            ...
            return (
                <div className={classes.tag} key={tag.key}>
                    <Form.Item
                        ...
                        rules={[
                            { required: true, whitespace: true, message: 'Please enter a tag' },
